...
header('Access-Control-Allow-Origin: *'); // Mengizinkan semua domain untuk mengakses, sesuaikan dengan domain Anda jika diperlukan.
header('Access-Control-Allow-Methods: POST, OPTIONS');
header('Access-Control-Allow-Headers: Content-Type');

// --- PENTING: GANTI DENGAN KUNCI API ANDA ---
// Kunci ini tidak akan terlihat oleh pengguna akhir.
$apiKey = "AIzaSyD5w83borgoqmFbgHuF1DmrjXxILDCd1hM"; 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($httpCode === 200) {
        ...
        echo json_encode(['error' => 'Failed to call Gemini API', 'details' => $response]);
    }
    exit;
}
?>
...
<body class="bg-rose-50 text-gray-800 transition-colors duration-300">
            ...
            <section class="w-full max-w-4xl p-8 md:p-12 bg-rose-100 rounded-2xl shadow-lg text-center transform hover:scale-105 transition-transform duration-300">
                <div class="flex flex-col items-center">
                    ...
                    <div class="text-md text-gray-500 leading-relaxed max-w-2xl mx-auto text-center">
                        <p>hallo! saya adalah Andre AI, siap membantu Anda kapan pun Anda mau! 😊</p>
                        <p class="mt-4">Tuliskan pertanyaan atau kebutuhan Anda, kami siap membantu Anda dengan senang hati. Klik ikon pesan untuk memulai percakapan dengan saya! 💬</p>
                        ...
                        <p class="mt-4">Jangan ragu untuk bertanya, saya siap membantu Anda! 😊</p>
                    </div>
                </div>
            </section>
    ...
    <script>
        ...
        const callAndreAi = async (prompt, imageData = null) => {
            showTypingIndicator();
            const selectedMode = aiModeSelector.value;
            
            let response;
            try {
                response = await fetch('chat-backend.php', {
                    method: 'POST',
        ...
        const sendMessage = async () => {
            ...
            if (imageFile) {
                ...
                reader.onload = async (e) => {
                    ...
                    imageData = imageUrl.split(',')[1]; // Dapatkan data Base64
                    displayMessage(userMessage || "Mengirim gambar...", 'user', imageUrl);
                    await callAndreAi(userMessage, imageData);
                };
